...
export class MnistData {
    ...
    async load(nTrain = 2000, nCalib = 200, nTest  = 600) {
        ...
        this.calibImages = this.datasetImages.slice(IMAGE_SIZE * nTrain, IMAGE_SIZE * (nTrain+nCalib))
        this.testImages = this.datasetImages.slice(IMAGE_SIZE * (nTrain+nCalib), IMAGE_SIZE * (nTrain+nCalib+nTest))
        this.trainLabels = this.datasetLabels.slice(0, N_CLASSES * nTrain)
        this.calibLabels = this.datasetLabels.slice(N_CLASSES * nTrain, N_CLASSES * (nTrain+nCalib))
        this.testLabels = this.datasetLabels.slice(N_CLASSES * (nTrain+nCalib), N_CLASSES * (nTrain+nCalib+nTest))
    ...
    */
    ...
    getCalibData() {
        const x_calib = tf.tensor4d(
                        this.calibImages,
                        [this.calibImages.length / IMAGE_SIZE, IMAGE_H, IMAGE_W, 1])
        const y_calib = tf.tensor2d(
                        this.calibLabels, 
                        [this.calibLabels.length / N_CLASSES, N_CLASSES])
